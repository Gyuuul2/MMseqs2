...
template <typename T> bool DBReader<T>::open(int accessType){
    ...
    // a run table database has no per entry index to read, only RunDbReader understands it
    if (getExtendedDbtype(dbtype) & Parameters::DBTYPE_EXTENDED_RUNS) {
        Debug(Debug::ERROR) << "Database " << dataFileName
                            << " stores its index as a run table and cannot be read here\n";
        EXIT(EXIT_FAILURE);
    }
