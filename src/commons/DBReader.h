...
public:
    ...
    // submits ids[0..n) into one lane and returns at once with how many fit the arena; awaitBatch makes them readable
    ...
    // start and await in lane 0, so a caller without a second lane to overlap still gets one submission per batch
...
private:
    ...
    // submits what the ring has room for and reaps completions; untilDone blocks until the lane is complete
