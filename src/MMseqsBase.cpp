...
std::vector<Command> baseCommands = {
        ...
        {"lin8-createdb",   lin8createdb,   &par.lin8createdb,   COMMAND_DATABASE_CREATION,
                "Convert FASTA file(s) to a length sorted, bit packed protein DB for linclust",
                "# Create a cluster database on one machine\n"
                "mmseqs lin8createdb file1.fa file2.fa sequenceDB --threads 20\n\n"
                "# Create it across several machines, then merge the parts\n"
                "mmseqs lin8createdb *.fa sequenceDB --node-list node01,node02 --threads 20\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:fastaFile1[.gz|.bz2]> ... <i:fastaFileN[.gz|.bz2]> <o:sequenceDB>",
                CITATION_MMSEQS2, {{"fast[a|q]File[.gz|bz2]", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA | DbType::VARIADIC, &DbValidator::flatfileAndStdin },
                                                           {"sequenceDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-clusthash",         lin8clusthash,         &par.lin8clusthash,         COMMAND_CLUSTER,
                "Remove exactly duplicated sequences from a run table database",
                "# One machine\n"
                "mmseqs lin8clusthash sequenceDB pairs --threads 20\n\n"
                "# The same work split over five, each writing its own pairs\n"
                "mmseqs lin8clusthash sequenceDB pairs.0 --node-count 5 --node-id 0 --threads 20\n\n"
                "# A later round starts from what the last one left\n"
                "mmseqs lin8clusthash sequenceDB pairs2 --valid pairs.valid --threads 20\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:sequenceDB> <o:clusterPairs>",
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                                           {"clusterPairs", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-extractkmers", lin8extractkmers, &par.lin8extractkmers, COMMAND_CLUSTER,
                "kmermatcher's first pass: the k-mers of every sequence, routed into hash buckets",
                "mmseqs lin8extractkmers sequenceDB kmers --kmer-per-seq 20 --threads 20\n\n"
                "# split over five machines, each writing its own chunks\n"
                "mmseqs lin8extractkmers sequenceDB kmers --node-count 5 --node-id 0 --threads 20\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:sequenceDB> <o:kmerRecords>",
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                                           {"kmerRecords", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-assignedpairs", lin8assignedpairs, &par.lin8assignedpairs, COMMAND_CLUSTER,
                "kmermatcher's second pass: a bucket sorted and grouped, and the pairs each group raises",
                "mmseqs lin8assignedpairs sequenceDB kmers pairs --threads 20\n\n"
                "# split over five machines, each taking every fifth bucket\n"
                "mmseqs lin8assignedpairs sequenceDB kmers pairs --node-count 5 --node-id 0\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:sequenceDB> <i:kmerRecords> <o:candidatePairs>",
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                                           {"kmerRecords", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"candidatePairs", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-pref", lin8pref, &par.lin8pref, COMMAND_CLUSTER,
                "kmermatcher's last pass: the pairs folded to one row a member, which is the pref linclust makes",
                "mmseqs lin8pref pairs sequenceDB pref --threads 20\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:candidatePairs> <i:sequenceDB> <o:prefPairs>",
                CITATION_MMSEQS2, {{"candidatePairs", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                                           {"prefPairs", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-align2clust", lin8align2clust, &par.lin8align2clust, COMMAND_CLUSTER,
                "Aligns each representative against its candidates, and on one machine assigns them too",
                "mmseqs lin8-align2clust sequenceDB pref aln clu_accepted --threads 20\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:sequenceDB> <i:prefPairs> <o:alignedPairs> <o:acceptedPairs>",
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                                           {"prefPairs", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"alignedPairs", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"acceptedPairs", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-align2clustmulti", lin8align2clustmulti, &par.lin8align2clustmulti, COMMAND_CLUSTER,
                "align2clust's clustering half: each sequence to the first representative that reached it",
                "mmseqs lin8-align2clustmulti aln pref clu_accepted\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:alignedPairs> <i:prefPairs> <o:acceptedPairs>",
                CITATION_MMSEQS2, {{"alignedPairs", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"prefPairs", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"acceptedPairs", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-mergehashredundancy", lin8mergehashredundancy, &par.lin8mergehashredundancy, COMMAND_CLUSTER,
                "mergeclusters's first half: the sequences the redundancy pass folded away, put back",
                "mmseqs lin8mergehashredundancy decided hash expanded\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:acceptedPairs> <i:redundancyPairs> <o:acceptedPairs>",
                CITATION_MMSEQS2, {{"acceptedPairs", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"redundancyPairs", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"expandedPairs", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::flatfile }}},
        {"lin8-createclusterdb", lin8createclusterdb, &par.lin8createclusterdb, COMMAND_CLUSTER,
                "mergeclusters's last half: the decided pairs as a clustering database",
                "mmseqs lin8createclusterdb clusters clu\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:acceptedPairs> <o:clusterDB>",
                CITATION_MMSEQS2, {{"acceptedPairs", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::flatfile },
                                                           {"clusterDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::clusterDb }}},
        {"lin8-createrepseqfasta",           lin8createrepseqfasta,       &par.lin8createrepseqfasta,       COMMAND_CLUSTER,
                "result2repseq and convert2fasta in one: the sequence that represents each cluster",
                "mmseqs lin8-createrepseqfasta sequenceDB clusterDB rep.fasta\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:sequenceDB> <i:clusterDB> <o:fastaFile>",
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
        ...
        {"lin8-createtsv", lin8createtsv, &par.lin8createtsv, COMMAND_CLUSTER,
                "Name a clustering of a run table database",
                "mmseqs lin8createtsv sequenceDB clu clu.tsv\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:sequenceDB> <i:clusterDB> <o:tsvFile>",
                CITATION_MMSEQS2, {{"sequenceDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                                           {"clusterDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::clusterDb },
        ...
        {"rbh",                  rbh,                  &par.searchworkflow,       COMMAND_MAIN,
                ...
                CITATION_MMSEQS2, {{"queryDB", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA, &DbValidator::sequenceDb },
                                          ...
                                          {"alignmentDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::alignmentDb },
                                          {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
        {"lin8clust",      lin8clust,      &par.linclustoneshotworkflow, COMMAND_EASY,
                "One shot linear clustering: cluster a whole set of protein FASTA files in one pass, on one machine or many",
                "mmseqs lin8clust in.fa clusterDB tmp --min-seq-id 0.5 -c 0.9 --cov-mode 0 --threads 20\n\n"
                "# Many machines: run the same command on each with its own --node-id and a shared tmp\n"
                "mmseqs lin8clust *.fa clusterDB /shared/tmp --node-list super003,super004 --threads 128\n",
                "Gyuri Kim <gyuribio@snu.ac.kr>",
                "<i:fastaFile1[.gz|.zst]> ... <i:fastaFileN[.gz|.zst]> <o:clusterDB> <tmpDir>",
                CITATION_MMSEQS2|CITATION_LINCLUST, {{"fastaFile[.gz|.zst]", DbType::ACCESS_MODE_INPUT, DbType::NEED_DATA|DbType::VARIADIC, &DbValidator::flatfile },
                                                            {"clusterDB", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::clusterDb },
                                                            {"tmpDir", DbType::ACCESS_MODE_OUTPUT, DbType::NEED_DATA, &DbValidator::directory }}},
