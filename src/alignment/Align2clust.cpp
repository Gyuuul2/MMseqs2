...
#pragma omp parallel for schedule(static)
    ...
    size_t db_maxseqlen = (cluSeqDbr != nullptr)
        ...
        : seqDbr->getMaxSeqLen();
    // drawn by hand so a thread's queries are increasing, which the two held results need to stay deadlock free
...
#pragma omp parallel
    {
        ...
        // parses one query and submits its member reads, which then fly while the previous query is aligned
        ...
        // aligns a prepared query against the members its lane holds
