...
BlockAligner::BlockAligner(
    ...
    // Zero every bias buffer: when compBiasCorrection is off these are never filled but are
    // still read as the block-aligner positional bias, so uninitialized values would corrupt scores.
...
void BlockAligner::initQuery(Sequence* query){
    ...
    memset(queryRevNumSeq, 0, maxSequenceLength * sizeof(int8_t));
    // Reset the query bias each call so a compBias-off run (or the tail past queryLength) never
    // reuses a previous query's values.
    memset(queryCompBias, 0, maxSequenceLength * sizeof(int16_t));
    memset(queryCompBiasRev, 0, maxSequenceLength * sizeof(int16_t));
...
BlockAligner::align(
    ...
    // gappedLocalAlign returns early (without writing the CIGAR) when it fails or misses coverage;
    // don't walk the stale CIGAR in that case.
...
s_align BlockAligner::gappedLocalAlignForward(
    ...
    // Validate BEFORE tracing back: a failed x-drop yields SIZE_MAX (or 0) indices, and
    // block_cigar_aa_trace_xdrop would then walk from an invalid end cell.
    if (res.query_idx == SIZE_MAX || res.reference_idx == SIZE_MAX
        || res.query_idx == 0 || res.reference_idx == 0) {
        local_aln.score1 = 0.0f;
        local_aln.qCov = 0.0f;
    ...
    // res.query_idx / res.reference_idx are counts of query / reference chars in the alignment,
    // so the last aligned index is start + count - 1.
