...
#pragma omp parallel for schedule(dynamic, 1) num_threads(par.threads) reduction(+ : rows)
    for (size_t repRankBlock = 0; repRankBlock < repRankBlocks; repRankBlock++) {
        // the redundant additions for this block are small next to clu_accepted; sort them by rank
        ...
        bool haveRep = false;
        uint64_t lastRep = 0;
        // clu_accepted is already grouped by representative rank; splice the sorted additions in
