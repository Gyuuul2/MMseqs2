...
static void anchorHashes(const char *data, uint32_t length, uint32_t anchorK,
    ...
    for (uint32_t at = 0; at + anchorK <= length; at++) {
        ...
        spread ^= spread >> 29;
        // the lowest few, kept in order, so which anchors a sequence has is a function of the sequence
...
static bool sequencesMatch(const char *left, const char *right, uint32_t length, float identity) {
    if (identity >= 1.0f) {
        // memcmp is vectorised, and at one the count and the comparison ask the same question
...
public:
    HashPartitions(const std::string &prefix, size_t partitions, unsigned int threads)
        ...
        // a fixed byte budget, not a fixed depth: a per partition depth would grow with what it divides
    ...
    // the file is read once and then gone, so its pages are dropped rather than left to be reclaimed
...
private:
    ...
    // the low bits pick the partition nowhere else, so the top bits keep equal hashes together
...
int lin8clusthash(int argc, const char **argv, const Command &command) {
    ...
    if (FileUtil::fileExists(nodeDonePath(par.db2, node.index).c_str()) == false) {
        ...
        if (crowded > 0) {
            Debug(Debug::INFO) << crowded << " anchors were shared by more than " << ANCHOR_BUCKET_MAX
                               << " sequences of one length and were left out\n";
        }
    ...
    for (unsigned int other = 0; other < node.count; other++) {
        if (FileUtil::fileExists((par.db2 + "." + SSTR(other)).c_str()) == false) {
            Debug(Debug::INFO) << "Wrote " << shard << ", rerun once every node has written its own\n";
            delete reduced;
    reader.close();
            return EXIT_SUCCESS;
        }
