...
CREATEDB_PAR=${CREATEDB_PAR:---write-lookup 0 --createdb-mode 0}
...
resolve_chunk_filelist() {
    ...
    local mem _cnt i=0 local_ref base ext
    : > "$out_list"
    # counted manifests carry "path<TAB>seqs"; the count stays in the manifest and only the path is staged
    while IFS=$'\t' read -r mem _cnt || [[ -n "${mem:-}" ]]; do
        [[ -z "${mem:-}" || "$mem" =~ ^[[:space:]]*# ]] && continue
        base=$(basename "$mem")
        ext=""
        ...
        # createdb here takes a file of paths, so the measured count stays in the manifest and out of the list
        printf '%s\n' "$local_ref" >> "$out_list"
        i=$((i + 1))
    done < <(stream_manifest "$filelist")
